		...
		const struct tcphdr *tcph, uint32_t seq, uint16_t window)
{
	...
	// PowerTCP Implementation
	cur_stream->rcvvar->ts_prev_lastack_rcvd = -1;
	cur_stream->sndvar->powertcp_cwnd = cur_stream->sndvar->cwnd;
	cur_stream->sndvar->row_smooth = 1;
		...
		struct tcphdr *tcph, uint32_t seq, uint32_t ack_seq, uint16_t window)
{
	...
	cur_stream->rcvvar->ts_prev_lastack_rcvd = -1;
	cur_stream->sndvar->powertcp_cwnd = cur_stream->sndvar->cwnd;
	cur_stream->sndvar->row_smooth = -1.0;
		...
		uint16_t window, int payloadlen)
{
	...
	if (dup && cur_stream->rcvvar->dup_acks == 3) {
		...
		printf("PETER, fast retransmission: cwnd %d \n", sndvar->cwnd);
	...
	if (rmlen > 0) {
		...
		printf("============================= \n");
		printf("PETER: ack_seq %u, last ack recv %u, cwnd %u, mss %u, packets %d \n", ack_seq,cur_stream->rcvvar->last_ack_seq, sndvar->cwnd, sndvar->mss, packets);
        //printf("PETER: mss %d \n", sndvar->mss);
		//printf("PETER: packets %d \n", packets);
		printf("PETER: srtt %u, ts_prev_lastack_rcvd %u, ts_lastack_rcvd %u \n", (cur_stream->rcvvar->srtt), cur_stream->rcvvar->ts_prev_lastack_rcvd, cur_stream->rcvvar->ts_lastack_rcvd);
		//printf("PETER: cur_ts %d \n", cur_ts);
		//printf("PETER: ts_lastack_rcvd %d \n", cur_stream->rcvvar->ts_lastack_rcvd);

		// Implement Here Theta Power TCP	
		if (cur_stream->state >= TCP_ST_ESTABLISHED) {
			float base_rtt = 15.0;  // 80ms as set by TC on switch
			
			// Get CWND
			uint32_t cwn_old = cur_stream->sndvar->powertcp_cwnd ;
			
			// Calculate Delta timestamps
			float dt;
			if (cur_stream->rcvvar->ts_prev_lastack_rcvd == -1) {
				dt = cur_stream->rcvvar->srtt;
			} else {
				dt = cur_stream->rcvvar->ts_lastack_rcvd - cur_stream->rcvvar->ts_prev_lastack_rcvd;
			}
			...
			// Get NormPower
			float theta = ((cur_stream->rcvvar->srtt) - cur_stream->rcvvar->prev_srtt ) / dt;
			float row_norm = ((theta + 1) * cur_stream->rcvvar->srtt) / base_rtt;
			printf("PETER: prev_srtt %u, dt %f, theta %f, row_norm %f \n", cur_stream->rcvvar->prev_srtt, dt, theta, row_norm);

			// Get Smoothed Power
			if (cur_stream->sndvar->row_smooth == -1.0) {
				cur_stream->sndvar->row_smooth = row_norm;
			}
			cur_stream->sndvar->row_smooth = (cur_stream->sndvar->row_smooth * (base_rtt * dt) + (row_norm * dt))/base_rtt;

			// Udpate Window
			// Bandwidth is channel capacity ~= 1000 mbit/s
			float beta = packets * sndvar->mss * base_rtt / 1;
			cur_stream->sndvar->powertcp_cwnd = (uint32_t) 0.9 * ((cwn_old / cur_stream->sndvar->row_smooth) + beta) + 0.1 * cwn_old;
			
			printf("PETER: PowerTCP cwnd %u, beta %f, row_smooth %f \n", cur_stream->sndvar->powertcp_cwnd, beta, cur_stream->sndvar->row_smooth);

			// Update Old Variables
			cur_stream->rcvvar->ts_prev_lastack_rcvd = cur_stream->rcvvar->ts_lastack_rcvd;
			cur_stream->rcvvar->prev_srtt = cur_stream->rcvvar->srtt;
			printf("PETER: new prev_srtt %u, new prev_lastack_rcvd %u \n", cur_stream->rcvvar->prev_srtt, cur_stream->rcvvar->ts_prev_lastack_rcvd);
		}	
